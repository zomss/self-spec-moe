...
def bench_run(
    ...
    repack_supported = group_size in MARLIN_SUPPORTED_GROUP_SIZES and (
        quant_type not in (scalar_types.float8_e4m3fn, scalar_types.float4_e2m1f)
    )  # p74: fp8/fp4 have no gptq repack kernel
    allspark_supported = (
    ...
    def gen_repack_params():
        ...
        # fp8/fp4 have no GPTQ repack path (p74: skip so the marlin_gemm vs
        # pytorch_gemm timers still run for float8_e4m3fn).
        if repack_supported and quant_type not in (
    ...
    results.append(
        benchmark.Timer(
            stmt="output = marlin_gemm(a, None, marlin_q_w, None, marlin_s, None, marlin_s2, marlin_zp, marlin_g_idx, marlin_sort_indices, marlin_workspace.scratch, quant_type, size_m, size_n, size_k, is_k_full, False, False, False)",  # noqa: E501
            globals=globals,
            label=label,
            sub_label=sub_label,
    ...
    results.append(
        benchmark.Timer(
            stmt="output = marlin_gemm(a, None, marlin_q_w, None, marlin_s, None, marlin_s2, marlin_zp, marlin_g_idx, marlin_sort_indices, marlin_workspace.scratch, quant_type, size_m, size_n, size_k, is_k_full, False, True, False)",  # noqa: E501
            globals=globals,
            label=label,
            sub_label=sub_label,
