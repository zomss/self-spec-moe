...
and the benefit is measured -- on a comm-bound (socket) testbed, lossless 2.5-3.1x
(beta 0.92), **but the real PCIe-link calibration revises this DOWN: sockets'
high f was TCP-latency-inflated; real PCIe is bandwidth-bound, giving ~1.2-1.4x at
serving batch and ~1x at low batch** (still lossless, a throughput effect). Remaining
gaps (exact PCIe f under contention, DBO baseline, full distributed driver) are
hardware-gated and need a real PCIe/multi-node rental. The
...
- *Benefit side (Phase 24, the gap that is now closed):* on a real comm-bound testbed
  ...
  **socket** point (B2b-pragmatic; overhead ~1.5%). **Real PCIe-link calibration
  (Phase 24 3d) revises this down**: PCIe is 7-14x slower than NVLink (measured: 55 /
  27.5 vs 389 GB/s) but the socket f=0.84 was TCP-latency-inflated; real PCIe is
  bandwidth-bound -> f ~0.05-0.1 at low batch (compute-bound, weak lever) rising to
  ~0.3-0.5 at serving batch -> **lossless ~1.2-1.4x (beta 0.92) at serving batch,
  ~1x at low batch**. The socket 2.5-3.1x is an optimistic upper bound; the realistic
  PCIe win is a modest throughput-regime effect.
