...
run_one() {  # <lever> <karm> <draft> <batches> <extra>
  ...
  env CUDA_VISIBLE_DEVICES=$GPU COMPILE_MODEL="$MODEL" \
      ...
      >> "$PHASE/logs/backfill_dense.log" 2>&1 \
...
# 1) the diagnosis test first: win512/k2 at b<=64
run_one win512 k2 "$MODEL" "$B64" "$(winstack 512)"
# 2) remaining window family at b<=64
for k in k4 k6; do run_one win512 $k "$MODEL" "$B64" "$(winstack 512)"; done
for k in k2 k4; do run_one win128 $k "$MODEL" "$B64" "$(winstack 128)"; done
