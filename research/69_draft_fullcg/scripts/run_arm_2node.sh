...
# cleanup, foreign-GPU check, log suffix) and P69_EXTRA passthrough for the
...
case "$ARM" in
  nospec)
    MODE=nospec; ENV=$ENVEA; PORT=16400; TAG=q30b_p69_nospec
    OVR="$COMMON" ;;
  b_rep)
    MODE=spec; ENV=$ENVSS; PORT=16500; TAG=q30b_p69_brep
...
foreign_gpu_busy() {
  ...
  for p in $(nvidia-smi --query-compute-apps=pid --format=csv,noheader \
    ...
    if [ -n "$u" ] && [ "$u" != "$me" ]; then
      echo "[p69] foreign GPU pid=$p user=$u on h107"; return 0
    fi
  done
  while read -r line; do
    p="${line%% *}"; u="${line##* }"
    if [ -n "$u" ] && [ "$u" != "$p" ] && [ "$u" != "$me" ]; then
      echo "[p69] foreign GPU pid=$p user=$u on $PEER"; return 0
    fi
...
for try in $(seq 1 "$RETRIES"); do
  while foreign_gpu_busy; do
    echo "[p69] $ARM K=$K: waiting 120s for foreign GPU work ($(date +%H:%M:%S))"
    sleep 120
  done
  kill_stragglers
...
W7_MASTER_PORT=$((PORT+try*7)) ${NCCLDBG:+NCCL_DEBUG=INFO}"
  echo "[p69] $ARM K=$K b=$BATCHES try=$try to=${TRY_TO}s peer=$PEER ($(date +%H:%M:%S))"
  ...
  echo "[p69] $ARM K=$K try=$try -> ${got:-0}/$nb batch rows ($(date +%H:%M:%S))"
  grep -hE "W7-2N|kv-window|KV cache size|shared-KV" "$LOGH" 2>/dev/null | tail -12
  if [ "${got:-0}" -ge "$nb" ]; then ok=1; break; fi
done
kill_stragglers
[ "$ok" = "1" ] || { echo "[p69] $ARM K=$K INCOMPLETE after $RETRIES tries"; exit 1; }
echo "[p69] $ARM K=$K done ($(date +%H:%M:%S))"
