...
for arm in "$@"; do
  ...
  if [ "$arm" = policy ]; then
    export VLLM_SELF_SPEC_ACCEPT_OFF_THRESHOLD=0.84 VLLM_SELF_SPEC_ACCEPT_ON_THRESHOLD=0.86 VLLM_SELF_SPEC_ACCEPT_PROBE_INTERVAL=128 VLLM_SELF_SPEC_ACCEPT_PROBE_BURST=1 VLLM_SELF_SPEC_SHORTCTX_OFF=2500:13
  else
    unset VLLM_SELF_SPEC_ACCEPT_OFF_THRESHOLD
  fi
