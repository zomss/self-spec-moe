#!/usr/bin/env python3
"""Measure one-token speculative acceptance for top-n local MoE drafts."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--draft-top-k-values", type=parse_int_list, default=[1, 2, 4])
...
def build_placements(
    *,
    ...
    return {
        ...
        "random": random,
        "contiguous_hot_replicated": [
            set(experts).union(hot_experts) for experts in contiguous
        ],
        "random_hot_replicated": [
            set(experts).union(hot_experts) for experts in random
        ],
    }
    ...
    *,
    ...
    def make_forward(module):
        def forward(self, hidden_states):
            ...
            mask[list(allowed_experts)] = False
            ...
            effective_top_k = min(draft_top_k, len(allowed_experts))
...
def speculative_acceptance_metrics(
    ...
    accepted = []
    for prompt_idx in range(p.shape[0]):
        ...
        p_values = p[prompt_idx, draft_tokens]
        q_values = q[prompt_idx, draft_tokens].clamp_min(1e-30)
        accept_prob = torch.minimum(torch.ones_like(p_values), p_values / q_values)
        draws = torch.rand(sample_count, generator=generator)
        accepted.append((draws < accept_prob).float().mean())
    ...
    return {
        "expected_acceptance_overlap": float(overlap.mean()),
        "sampled_acceptance_rate": float(torch.stack(accepted).mean()),
...
def main() -> int:
    ...
    if num_experts is None:
        raise ValueError("This script currently expects config.num_experts")
    ...
    for placement_name, local_experts_by_rank in placements.items():
        for rank, local_experts in enumerate(local_experts_by_rank):
            for draft_top_k in args.draft_top_k_values:
                with patched_qwen_routers(
                    model,
                    allowed_experts=local_experts,
                    draft_top_k=draft_top_k,
                ):
                    draft_logits = next_token_logits(model, tokenizer, prompts, args)
                metrics = speculative_acceptance_metrics(
                    full_logits,
                    draft_logits,
                    args.sample_count,
                    args.seed + 1000 * rank + draft_top_k,
                )
                rows.append(
                    {
