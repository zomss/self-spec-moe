#!/usr/bin/env python3
"""OFF-region hardening: exhaustive priced search over the FULL combo space.
...
# the map's OFF(-or-marginal) cells: mla everywhere; moe short-ctx band
OFF_CELLS = {
...
def main() -> int:
    ...
    lines = ["# OFF-region hardening — exhaustive combo search, optimistic pricing",
        ...
        for b, ck in cells:
            ...
            for combo in cands(COMPONENTS[group]):
                ...
                if r is None and len(combo) == 1:
                    arm = {("moe", "win512"): "m_win", ("moe", "win128"): "m_win128",
    ...
    text = "\n".join(lines)
    (PHASE / "data/off_hardening.md").write_text(text + "\n")
    print(text)
    return 0
