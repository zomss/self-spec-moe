...
| **W3** | **Pre-register** the threshold that justifies Round 2 + hidden switching, in decode-only currency | W2 | 0 | **DONE 2026-08-05** (`w3_preregistration.md`, binding). Currency: S_dec judges, S_e2e reports. Protocol: episode rejection + b1 second-highest rule + cross-boot certification; noise floor 1.0% measured. W6 gate: mean(T−B) ≥ +2% S_dec or single-regime +5% at mean ≥ 0; W7b additionally needs amortized transition < 0.5%. Screening: window-only envelope +0.3%/+0.9% is BELOW gate — the thesis rides on full composition; W5 must PREDICT a pass from audited cells before W6 runs. Fail outcome pre-declared: static + gate is the deliverable |
| **W4** | Complete the throughput model (I5): prefill term, per-cell parked cost, per-transition cost. Probe discriminator already written (95/`run_e1p_probe.sh`) | W1 | ~2 GPU-h | **DONE 2026-08-05** (`results_w4.md`) except W4c. W4a: per-flip transition cost ≈ 0 — the ~20ms story was duty misattributed; wrong-arm duty = 1.06 step-equiv/armed step; parked −0.6% b16 / ~0 b8; C-C already met for K/OFF. W4b: lottery-inflation REFUTED (audit/table R mean 1.00×) — F6's 1.46× was a currency artifact (corrected, T11). Real decomposition: R carries a compile-vs-serving PROTOCOL gap (map understates long-ctx serving by ~35% at c14k); f carries a compile-content gap (C4 accepts 4.1 where live R4 content accepts 2.3). **W4c DONE 2026-08-06**: the gap is REALIZATION × DECODE-LENGTH, not protocol arithmetic (within-realization protocol axis +5%; N-bias ~10%). At R5-natural (~72-tok answers) scratchpad 0.846 vs piecewise 1.420 — a 68% swing on an axis the compile protocol cannot see (ignore_eos + non-stopping content). Realization joins the composition pool as a boot-class lever; deployed C3 stack is wrong for short-decode regimes. DISCLOSED: 96's llama serving runs before W4c ran piecewise while phase-95/map ran scratchpad (env divergence, caught by W4c's anchor check). Compile-R shortlists, never decides |
| **W5** | Round 1 redesign: interpolate R, emit tie-sets with uncertainty (S1, I3) | W4 | — | **DONE 2026-08-06** (`w5_round1.md` + `results_w5.md`). Tie-sets emitted (sizes 1–37; R4/R8 collapse to {OFF}); boot-class compression → Round 2 costs ~2 boots/arch. Separability partial (dense ≤8%; llama FAILS at R5cot 35% — content×composition interaction; R4's ρ=0.41 both arches = the content gap quantified). Calibration: long-ctx intervals cover live values; short-ctx (R1/R6) MISCALIBRATED — no map cells below c2000. **W6 verdict: dense letter-PASS (R1 +5.2%) but pessimistic read fails all regimes and R1 is a miscalibrated cell → W6 = targeted interval-shrinkage measurement (Round 2 proper); llama FAIL → deliverable is static q-w4a16_w-2048_s-b2 piecewise + OFF gate** |
