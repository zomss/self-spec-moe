...
MODEL="Qwen/Qwen3-8B"; DRAFT="$HOME/ckpts/Qwen3-8B-W4A8-gptq"
...
lane0() {
  boot 0 off off "" 0 "" "$PHASE/data/w13/w13_off.json"
  for i in 0 1 2; do
    IFS='|' read -r n sk win <<< "${COMPS[$i]}"
    [ "$sk" = "none" ] && sk=""
    boot 0 "prof_$n" "$DRAFT" "$sk" "$win" \
         "$PHASE/data/w13/prof_$n" "$PHASE/data/w13/prof_$n/serving.json"
    boot 0 "unc_$n" "$DRAFT" "$sk" "$win" "" \
...
lane1() {
  for i in 3 4; do
    IFS='|' read -r n sk win <<< "${COMPS[$i]}"
    [ "$sk" = "none" ] && sk=""
    boot 1 "prof_$n" "$DRAFT" "$sk" "$win" \
         "$PHASE/data/w13/prof_$n" "$PHASE/data/w13/prof_$n/serving.json"
    boot 1 "unc_$n" "$DRAFT" "$sk" "$win" "" \
         "$PHASE/data/w13/w13_unc_$n.json"
  done
  cleanup 1
}
