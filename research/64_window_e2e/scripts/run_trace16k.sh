#!/bin/bash
# Phase 64: torch-profiler trace of the W512 self-draft decode cycle at 16k
# on the 2-node fabric. Same launch/guard pattern as run_arm.sh (bounded h106
# wait, own-pattern kills, foreign-GPU guard). Rank 0 (h107) writes the trace.
# Usage: run_trace16k.sh [K=2] [batch=8] [trace_len=60] [try_to_s=1500]
set -u
K="${1:-2}"
BATCH="${2:-8}"
TRACE_LEN="${3:-60}"
...
TRACE_DIR="$PHASE/data/trace_w512k${K}_b${BATCH}"
...
foreign_gpu_busy() {
  ...
  for p in $(nvidia-smi --query-compute-apps=pid --format=csv,noheader \
    ...
    if [ -n "$u" ] && [ "$u" != "$me" ]; then
      echo "[p64-trace] foreign GPU pid=$p user=$u on h107"; return 0
    fi
  done
  while read -r line; do
    p="${line%% *}"; u="${line##* }"
    if [ -n "$u" ] && [ "$u" != "$p" ] && [ "$u" != "$me" ]; then
      echo "[p64-trace] foreign GPU pid=$p user=$u on h106"; return 0
    fi
...
while foreign_gpu_busy; do
  echo "[p64-trace] waiting 120s for foreign GPU work ($(date +%H:%M:%S))"
  sleep 120
done
kill_stragglers
...
W7_GPU_MEM=0.90 W7_MASTER_PORT=14500 VLLM_CUSTOM_SCOPES_FOR_PROFILING=1 \
W7_DRAFT_QUANT= W7_DRAFT_FULL_REPLICA=0 W7_DRAFT_LOCAL_ROUTE=0 \
W7_DRAFT_NODE_LOCAL=0 VLLM_SELF_SPEC_DRAFT_KV_WINDOW=512 \
VLLM_SELF_SPEC_DRAFT_KV_SINKS=16 W7_KV_WINDOW_DEBUG=1"
LOGH="$PHASE/logs/trace_w512k${K}_b${BATCH}.log"
LOG6="$PHASE/logs/trace_w512k${K}_b${BATCH}_h106.log"
echo "[p64-trace] K=$K b=$BATCH len=$TRACE_LEN to=${TRY_TO}s ($(date +%H:%M:%S))"
...
echo "[p64-trace] done RC=$RC ($(date +%H:%M:%S))"
