...
"""X11 -- close the two gaps X10 left: install Conch, and sweep Marlin's flags.

DIAGNOSTIC, NOT SCORED.

X10 found Humming fastest, Marlin second, Machete (today) third. It left two
gaps that were environment or configuration artifacts rather than measurements:
Conch was excluded only because the package was not installed, and Marlin ran at
whatever flags its defaults pick. This closes both.

Marlin flags. `use_atomic_add` is chosen by `should_use_atomic_add_reduce`,
which returns False when `n >= 2048` -- true for ALL four of our projections
(n = 4096..24576), so it is off regardless of `VLLM_MARLIN_USE_ATOMIC_ADD`.
Forcing it on tests whether that heuristic is right for us. `use_fp32_reduce`
defaults to True; forcing it off trades reduction precision for speed.

Original X10 rationale follows.
...
def main() -> int:
    ...
    # (class, forced marlin flags or None)
    KERNELS = {
        "machete": (MacheteLinearKernel, None),
        "marlin": (MarlinLinearKernel, None),
        "marlin_atomic": (MarlinLinearKernel, {"use_atomic_add": True}),
        "marlin_nofp32": (MarlinLinearKernel, {"use_fp32_reduce": False}),
        "marlin_atomic_nofp32": (
            MarlinLinearKernel,
            {"use_atomic_add": True, "use_fp32_reduce": False},
        ),
        "humming": (HummingLinearKernel, None),
        "conch": (ConchLinearKernel, None),
        "triton_w4a16": (TritonW4A16LinearKernel, None),
    }
    ...
    for name, k, n in LAYER_SHAPES:
        for m in M_VALUES:
            ...
            for label, (cls, marlin_flags) in KERNELS.items():
                try:
                    ...
                    # marlin_utils calls ops.marlin_gemm by module attribute, so
                    # overriding it here forces flags the default path never sets.
                    restore: tuple | None = None
                    if marlin_flags:
                        import vllm._custom_ops as _ops

                        original_gemm = _ops.marlin_gemm

                        def forced(*a, _o=original_gemm, _f=marlin_flags, **kw):
                            kw.update(_f)
                            return _o(*a, **kw)

                        _ops.marlin_gemm = forced
                        restore = (_ops, original_gemm)
                    ...
                    try:
                        entry["us_per_call"][label] = time_graph(
                            call, GRAPH_CALLS, REPLAYS
                        )
                    finally:
                        # MUST restore: without this the forced flags leak into
                        # every later variant and the labels stop meaning what
                        # they say (nofp32 would silently also carry atomic).
                        if restore is not None:
                            restore[0].marlin_gemm = restore[1]
    ...
    args.output.write_text(
        json.dumps(
            {
                "schema_version": 1,
                "record_type": "w98_w4a16_final_sweep",
                "model": "Qwen3-8B",
                "group_size": GROUP_SIZE,
