...
Phase-98 D2 COPY of research/88_regime_eval/scripts/regime_datasets.py.
The single difference is the c4 cache root, which is parameterised by
W98D2_HF_HOME instead of hard-coded to the original box. Copied rather
than edited in place because the original is hash-pinned by the frozen
seeds 2-3 freeze and must not move.
"""
import glob
import os
...
def _c4_docs(tok, n, target_tok, skip=0):
    ...
    import json
    import os
    # Phase-98 D2 relocation: the canonical loader hard-codes the original
    # box's cache root, which does not exist on h104. Only the ROOT moves --
    # the glob, ordering, packing and every downstream operation are
    # byte-identical to research/88_regime_eval/scripts/regime_datasets.py.
    _c4_root = os.environ.get(
        "W98D2_HF_HOME", os.path.expanduser("~/.cache/huggingface")
    )
    for path in sorted(glob.glob(
            f"{_c4_root}/hub/datasets--allenai--c4/**/*.json.gz",
            recursive=True)):
