#!/usr/bin/env python3
"""Validate the source-bound Phase 97 repaired-prefill V11 package."""
...
class B0RunAuthorizationV11Error(ValueError):
    """Raised when the V11 package drifts or inflates authority."""


def _require(condition: bool, message: str) -> None:
    if not condition:
        raise B0RunAuthorizationV11Error(message)
...
def _load_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise B0RunAuthorizationV11Error(
...
def _validate_schema(authorization: Mapping[str, Any]) -> None:
    ...
    try:
        ...
        raise B0RunAuthorizationV11Error(
            f"invalid V11 authorization schema: {exc}"
        ) from exc
    errors = sorted(
    ...
    if errors:
        ...
        raise B0RunAuthorizationV11Error(
            f"V11 authorization schema rejected {path}: {first.message}"
        )


def validate_authorization_v11(
    authorization: Mapping[str, Any],
) -> dict[str, Any]:
    """Validate V11 without creating its output or running a GPU command."""
    ...
    try:
        ...
        output_dir = (REPO_ROOT / V11_OUTPUT_PATH).resolve()
        dispatch_output = _reviewed_output_path(REPO_ROOT / V11_AUTHORIZATION_PATH)
        specs = build_boot_specs(effective, output_dir)
    except P4RunnerError as exc:
        raise B0RunAuthorizationV11Error(
            f"live runner rejects the V11 package: {exc}"
        ) from exc

    _require(
        dispatch_output == output_dir,
        "V11 reviewed authorization does not dispatch to its fresh output",
    )
    cells = [cell for spec in specs for cell in spec["plan"]["cells"]]
    _require(
        len(specs) == 9 and len(cells) == 432,
        "V11 runner no longer constructs the complete matrix",
    )
    _require(
        all(
        ...
        "a V11 boot spec lost a sampler or EngineCore policy",
    )
    _require(
        all(
            spec["engine"]["max_num_batched_tokens"] == SERVING_MAX_NUM_BATCHED_TOKENS
            and spec["engine"]["enable_chunked_prefill"] is True
            and spec["engine"]["gpu_memory_utilization"]
            == SERVING_GPU_MEMORY_UTILIZATION
            and "full_prefill_budget" not in spec
            and spec["chunked_prefill_budget"]["effective_scheduler_token_budget"]
            == SERVING_EFFECTIVE_MAX_NUM_SCHEDULED_TOKENS
            and spec["chunked_prefill_budget"]["full_microbatch_prefill_allowed"]
            is False
            and spec["chunked_prefill_budget"]["capture_cohort_barrier_required"]
            is True
            for spec in specs
        ),
        "a V11 boot spec lost the bounded cohort-barrier engine contract",
    )
    _require(
        all(
            _total_output_tokens(cell) == cell["generation"]["max_output_tokens"] + 1
            and cell["generation"]["requested_output_tokens"]
            == len(cell["generation"]["prompt_record_ids"])
            * cell["generation"]["max_output_tokens"]
            for cell in cells
        ),
        "a V11 cell lost the prefill-sample/decode-work separation",
    )

    planned_cohort_count = 0
    for cell in cells:
        prompt_ids = cell["generation"]["prompt_record_ids"]
        batch = cell["generation"]["batch"]
        _require(
            len(prompt_ids) == 32 and 32 % batch == 0,
            "a V11 capture cell has invalid prompt slicing",
        )
        for start in range(0, len(prompt_ids), batch):
            ...
            _require(
                marker["contract_id"] == "p4-capture-cohort-barrier-v1"
                and marker["measurement_only"] is True
                and marker["frozen_request_ids"] == members
                and marker["action_id"] == cell["matrix"]["action_id"]
                and marker["measured_decode_tokens"]
                == cell["generation"]["max_output_tokens"]
                and marker["unmeasured_prefill_tokens"] == 1,
                "a V11 prompt slice lost its exact cohort marker",
            )
            planned_cohort_count += 1
    _require(
        planned_cohort_count == 3384,
        "V11 planned capture cohorts do not close to 3,384",
    )

    executable_dir = Path(environment["PATH"].split(os.pathsep, maxsplit=1)[0])
    _require(
        executable_dir == Path(sys.prefix) / "bin",
        "V11 preflight did not prepend the active environment",
    )
    _require(
        not (REPO_ROOT / V11_OUTPUT_PATH).exists(),
        "V11 validation created the authorized output",
    )
    return {
        "status": "pass",
        ...
        "chunked_prefill_probe_status": "pass",
        "chunked_prefill_probe_scored": False,
        "chunked_prefill_probe_action_id": "target-matching-k4",
        "chunked_prefill_probe_regimes": ["R4", "R5", "R5cot"],
        "chunked_prefill_probe_cohorts": 3,
        "chunked_prefill_probe_requests": 24,
        "chunked_prefill_probe_shared_kv_blocks": 24527,
        ...
        "repair_parent_rejection_classification": "observer_cardinality_bug",
        "repair_parent_rejection_caused_gpu_case_failure": False,
        "repair_isolated_measured_event_count": 610,
        "repair_transition_measured_event_count": 1273,
        "repair_isolated_trace_record_count": 612,
        "repair_transition_trace_record_count": 1289,
        ...
        "capture_count": 432,
        "planned_capture_cohort_count": planned_cohort_count,
...
def main() -> int:
    """Validate one V11 package and emit its non-execution summary."""
    args = parse_args()
    result = validate_authorization_v11(_load_json(args.authorization))
