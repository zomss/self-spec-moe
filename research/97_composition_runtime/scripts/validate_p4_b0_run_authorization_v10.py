#!/usr/bin/env python3
"""Validate the source-bound Phase 97 bounded chunked-prefill V10 package."""

from __future__ import annotations

import argparse
import json
import os
import sys
from collections.abc import Mapping, Sequence
from pathlib import Path
from typing import Any

from jsonschema import Draft202012Validator
from run_p4_b0_value_screen import (
    GPU4_UUID,
    INPROCESS_ENGINE_CORE_CLASS,
    MINIMUM_SHARED_KV_BLOCKS,
    NATIVE_SAMPLER_ENV,
    NATIVE_SAMPLER_VALUE,
    SERVING_EFFECTIVE_MAX_NUM_SCHEDULED_TOKENS,
    SERVING_GPU_MEMORY_UTILIZATION,
    SERVING_MAX_NUM_BATCHED_TOKENS,
    V1_MULTIPROCESSING_ENV,
    V1_MULTIPROCESSING_VALUE,
    V10_AUTHORIZATION_PATH,
    V10_OUTPUT_PATH,
    P4RunnerError,
    _boot_child_environment,
    _capture_cohort_metadata,
    _preflight_inprocess_engine_core,
    _preflight_native_sampler,
    _reviewed_output_path,
    _total_output_tokens,
    build_boot_specs,
    resolve_authorization_package,
    validate_execution_authority,
)

PHASE_DIR = Path(__file__).resolve().parents[1]
REPO_ROOT = PHASE_DIR.parents[1]
SCHEMA_PATH = PHASE_DIR / "schemas" / "p4_b0_run_authorization_v10.schema.json"


class B0RunAuthorizationV10Error(ValueError):
    """Raised when the V10 package drifts or inflates authority."""


def _require(condition: bool, message: str) -> None:
    if not condition:
        raise B0RunAuthorizationV10Error(message)


def _load_json(path: Path) -> dict[str, Any]:
    try:
        value = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise B0RunAuthorizationV10Error(
            f"cannot load JSON artifact {path}: {exc}"
        ) from exc
    _require(isinstance(value, dict), f"JSON artifact must be an object: {path}")
    return value


def _format_json_path(parts: Sequence[Any]) -> str:
    path = "$"
    for part in parts:
        path += f"[{part}]" if isinstance(part, int) else f".{part}"
    return path


def _validate_schema(authorization: Mapping[str, Any]) -> None:
    schema = _load_json(SCHEMA_PATH)
    try:
        Draft202012Validator.check_schema(schema)
    except Exception as exc:
        raise B0RunAuthorizationV10Error(
            f"invalid V10 authorization schema: {exc}"
        ) from exc
    errors = sorted(
        Draft202012Validator(schema).iter_errors(authorization),
        key=lambda error: [str(part) for part in error.absolute_path],
    )
    if errors:
        first = errors[0]
        path = _format_json_path(list(first.absolute_path))
        raise B0RunAuthorizationV10Error(
            f"V10 authorization schema rejected {path}: {first.message}"
        )


def validate_authorization_v10(
    authorization: Mapping[str, Any],
) -> dict[str, Any]:
    """Validate V10 without creating its output or running a GPU command."""
    _validate_schema(authorization)
    try:
        effective = resolve_authorization_package(
            authorization,
            require_output_absent=True,
        )
        validate_execution_authority(effective)
        environment = _boot_child_environment({})
        sampler_evidence = _preflight_native_sampler(environment)
        engine_core_evidence = _preflight_inprocess_engine_core(environment)
        output_dir = (REPO_ROOT / V10_OUTPUT_PATH).resolve()
        dispatch_output = _reviewed_output_path(REPO_ROOT / V10_AUTHORIZATION_PATH)
        specs = build_boot_specs(effective, output_dir)
    except P4RunnerError as exc:
        raise B0RunAuthorizationV10Error(
            f"live runner rejects the V10 package: {exc}"
        ) from exc

    _require(
        dispatch_output == output_dir,
        "V10 reviewed authorization does not dispatch to its fresh output",
    )
    cells = [cell for spec in specs for cell in spec["plan"]["cells"]]
    _require(
        len(specs) == 9 and len(cells) == 432,
        "V10 runner no longer constructs the complete matrix",
    )
    _require(
        all(
            spec["environment"].get(NATIVE_SAMPLER_ENV) == NATIVE_SAMPLER_VALUE
            and spec["environment"].get(V1_MULTIPROCESSING_ENV)
            == V1_MULTIPROCESSING_VALUE
            for spec in specs
        ),
        "a V10 boot spec lost a sampler or EngineCore policy",
    )
    _require(
        all(
            spec["engine"]["max_num_batched_tokens"] == SERVING_MAX_NUM_BATCHED_TOKENS
            and spec["engine"]["enable_chunked_prefill"] is True
            and spec["engine"]["gpu_memory_utilization"]
            == SERVING_GPU_MEMORY_UTILIZATION
            and "full_prefill_budget" not in spec
            and spec["chunked_prefill_budget"]["effective_scheduler_token_budget"]
            == SERVING_EFFECTIVE_MAX_NUM_SCHEDULED_TOKENS
            and spec["chunked_prefill_budget"]["full_microbatch_prefill_allowed"]
            is False
            and spec["chunked_prefill_budget"]["capture_cohort_barrier_required"]
            is True
            for spec in specs
        ),
        "a V10 boot spec lost the bounded cohort-barrier engine contract",
    )
    _require(
        all(
            _total_output_tokens(cell) == cell["generation"]["max_output_tokens"] + 1
            and cell["generation"]["requested_output_tokens"]
            == len(cell["generation"]["prompt_record_ids"])
            * cell["generation"]["max_output_tokens"]
            for cell in cells
        ),
        "a V10 cell lost the prefill-sample/decode-work separation",
    )

    planned_cohort_count = 0
    for cell in cells:
        prompt_ids = cell["generation"]["prompt_record_ids"]
        batch = cell["generation"]["batch"]
        _require(
            len(prompt_ids) == 32 and 32 % batch == 0,
            "a V10 capture cell has invalid prompt slicing",
        )
        for start in range(0, len(prompt_ids), batch):
            members = prompt_ids[start : start + batch]
            marker = _capture_cohort_metadata(cell, members)
            _require(
                marker["contract_id"] == "p4-capture-cohort-barrier-v1"
                and marker["measurement_only"] is True
                and marker["frozen_request_ids"] == members
                and marker["action_id"] == cell["matrix"]["action_id"]
                and marker["measured_decode_tokens"]
                == cell["generation"]["max_output_tokens"]
                and marker["unmeasured_prefill_tokens"] == 1,
                "a V10 prompt slice lost its exact cohort marker",
            )
            planned_cohort_count += 1
    _require(
        planned_cohort_count == 3384,
        "V10 planned capture cohorts do not close to 3,384",
    )

    executable_dir = Path(environment["PATH"].split(os.pathsep, maxsplit=1)[0])
    _require(
        executable_dir == Path(sys.prefix) / "bin",
        "V10 preflight did not prepend the active environment",
    )
    _require(
        not (REPO_ROOT / V10_OUTPUT_PATH).exists(),
        "V10 validation created the authorized output",
    )
    return {
        "status": "pass",
        "package_id": authorization["package_id"],
        "authorization_decision": "approve",
        "approved_scope": authorization["decision"]["scope"],
        "gpu_executed": False,
        "prior_value_screen_attempts": 8,
        "latest_prior_complete_capture_count": 8,
        "latest_prior_incomplete_capture_count": 0,
        "latest_prior_gpu_executed": True,
        "latest_prior_score_emitted": False,
        "prior_outputs_preserved": True,
        "full_prefill_geometry_rejected": True,
        "chunked_prefill_probe_status": "pass",
        "chunked_prefill_probe_scored": False,
        "chunked_prefill_probe_action_id": "target-matching-k4",
        "chunked_prefill_probe_regimes": ["R4", "R5", "R5cot"],
        "chunked_prefill_probe_cohorts": 3,
        "chunked_prefill_probe_requests": 24,
        "chunked_prefill_probe_shared_kv_blocks": 24527,
        "chunked_prefill_cohort_cpu_proven": True,
        "chunked_prefill_k4_gpu_probe_passed": True,
        "launch_dispatch_tested": True,
        "decode_work_offset_tested": True,
        "request_id_canonicalization_tested": True,
        "internal_request_id_randomization_retained": True,
        "native_sampler_preflight": sampler_evidence,
        "engine_core_preflight": engine_core_evidence,
        "ninja_preflight_passed": True,
        "source_bindings_current": True,
        "gpu_physical_index": 4,
        "gpu_uuid": GPU4_UUID,
        "physical_boot_count": 9,
        "capture_count": 432,
        "planned_capture_cohort_count": planned_cohort_count,
        "max_num_batched_tokens": SERVING_MAX_NUM_BATCHED_TOKENS,
        "effective_max_num_scheduled_tokens": (
            SERVING_EFFECTIVE_MAX_NUM_SCHEDULED_TOKENS
        ),
        "gpu_memory_utilization": SERVING_GPU_MEMORY_UTILIZATION,
        "minimum_launch_capacity_blocks": MINIMUM_SHARED_KV_BLOCKS,
        "engine_core_class": INPROCESS_ENGINE_CORE_CLASS,
        "v10_execution_authorized": True,
        "value_screen_scoring_authorized": True,
        "score_grants_authority": False,
        "p4a_engineering_authorized": False,
        "action_admitted": False,
        "output_dir_create_new_ready": True,
        "next_artifact": "p4_b0_value_screen_result",
    }


def parse_args() -> argparse.Namespace:
    """Parse command-line arguments."""
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--authorization", type=Path, required=True)
    parser.add_argument("--out", type=Path)
    return parser.parse_args()


def main() -> int:
    """Validate one V10 package and emit its non-execution summary."""
    args = parse_args()
    result = validate_authorization_v10(_load_json(args.authorization))
    payload = json.dumps(result, indent=2, sort_keys=True) + "\n"
    if args.out is None:
        print(payload, end="")
    else:
        args.out.write_text(payload, encoding="utf-8")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
