#!/usr/bin/env python3
"""Validate the source-bound Phase 97 B0 GPU-4 retry authorization."""
...
class B0RunAuthorizationV3Error(ValueError):
    """Raised when the retry package drifts or inflates authority."""


def _require(condition: bool, message: str) -> None:
    if not condition:
        raise B0RunAuthorizationV3Error(message)
...
def _load_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise B0RunAuthorizationV3Error(
...
def _validate_schema(authorization: Mapping[str, Any]) -> None:
    ...
    try:
        ...
        raise B0RunAuthorizationV3Error(
            f"invalid V3 authorization schema: {exc}"
        ) from exc
    errors = sorted(
    ...
    if errors:
        ...
        raise B0RunAuthorizationV3Error(
            f"V3 authorization schema rejected {path}: {first.message}"
        )
...
def validate_authorization_v3(
    ...
    """Validate the retry approval and return its narrow authority summary.

    Args:
        authorization: Parsed V3 retry-authorization package.

    Returns:
        Machine-readable source, preflight, matrix, and authority summary.

    Raises:
        B0RunAuthorizationV3Error: If any approved input or boundary drifts.
    """
    ...
    return {
        ...
        "prior_attempt_capture_count": 0,
        "prior_attempt_scored": False,
        "prior_output_preserved": True,
        "launcher_repair_validated": True,
...
def main() -> int:
    """Validate one fresh retry package and print its summary."""
    args = parse_args()
    result = validate_authorization_v3(_load_json(args.authorization))
