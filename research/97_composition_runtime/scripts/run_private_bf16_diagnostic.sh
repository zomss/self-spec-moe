#!/bin/bash
# Phase 97: W4A8 + private-BF16-KV MBT2048 diagnostic control.
set -uo pipefail
...
GPU=4
MBT=2048
TIMEOUT_S="${P97_BF16_TIMEOUT_S:-900}"
...
CUDA_VISIBLE_DEVICES="$GPU" P97_GPU_PHYSICAL="$GPU" \
  ...
  --draft-execution compiled >"$LOG" 2>&1
status=$?
cleanup_sampler
...
if [ ! -s "$OUT" ]; then
  "$PYTHON" "$PHASE/scripts/record_warmup_timeout.py" \
    ...
    --draft-execution compiled --timeout-seconds "$TIMEOUT_S" \
