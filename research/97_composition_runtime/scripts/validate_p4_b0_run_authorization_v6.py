#!/usr/bin/env python3
"""Validate the source-bound Phase 97 B0 atomic-ingress V6 package."""
...
class B0RunAuthorizationV6Error(ValueError):
    """Raised when the atomic-ingress V6 package drifts or inflates authority."""


def _require(condition: bool, message: str) -> None:
    if not condition:
        raise B0RunAuthorizationV6Error(message)
...
def _load_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise B0RunAuthorizationV6Error(
...
def _validate_schema(authorization: Mapping[str, Any]) -> None:
    ...
    try:
        ...
        raise B0RunAuthorizationV6Error(
            f"invalid V6 authorization schema: {exc}"
        ) from exc
    errors = sorted(
    ...
    if errors:
        ...
        raise B0RunAuthorizationV6Error(
            f"V6 authorization schema rejected {path}: {first.message}"
        )


def validate_authorization_v6(
    authorization: Mapping[str, Any],
) -> dict[str, Any]:
    """Validate V6 without creating its output directory or running a GPU."""
    ...
    _require(
        ...
        "a V6 boot spec lost a sampler or EngineCore policy",
    )
    _require(
        all(
        ...
        "a V6 boot spec lost the measurement-only engine repair",
    )
    _require(
        authorization["atomic_ingress_proof"]["engine_core_class"]
        == INPROCESS_ENGINE_CORE_CLASS
        and authorization["atomic_ingress_proof"]["first_decode_query_widths"]
        == [1] * 8
        and authorization["atomic_ingress_proof"]["first_decode_exclusion_reasons"]
        == [],
        "V6 no longer binds the passing atomic-ingress geometry",
    )
    _require(
        authorization["atomic_ingress_proof"]["shared_target_kv_blocks"]
        >= MINIMUM_SHARED_KV_BLOCKS,
        "V6 atomic proof fell below the shared-KV launch floor",
    )
    _require(
        ...
        "V6 did not preserve the real-serving chunked-prefill boundary",
    )
    _require(
        {cell["generation"]["generation_seed"] for cell in cells} == {0},
        "V6 workload is no longer uniformly per-request seeded",
    )
    executable_dir = Path(environment["PATH"].split(os.pathsep, maxsplit=1)[0])
    _require(
        executable_dir == Path(sys.prefix) / "bin",
        "V6 preflight did not prepend the active environment",
    )
    _require(
        not (REPO_ROOT / V6_OUTPUT_PATH).exists(),
        "V6 validation created the authorized output",
    )
    return {
        "status": "pass",
        ...
        "atomic_proof_shared_kv_blocks": authorization["atomic_ingress_proof"][
            "shared_target_kv_blocks"
        ],
...
def main() -> int:
    """Validate one V6 package and print its non-execution summary."""
    args = parse_args()
    result = validate_authorization_v6(_load_json(args.authorization))
