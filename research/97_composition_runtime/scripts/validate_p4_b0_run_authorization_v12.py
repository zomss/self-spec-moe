...
"""Validate the source-bound Phase 97 block-parallel two-lane V12 package."""
...
SCHEMA_PATH = PHASE_DIR / "schemas" / "p4_b0_run_authorization_v12.schema.json"


class B0RunAuthorizationV12Error(ValueError):
    """Raised when the V12 package cannot prove its execution authority."""


def _require(condition: bool, message: str) -> None:
    if not condition:
        raise B0RunAuthorizationV12Error(message)
...
def _validate_spec_lanes(package: Mapping[str, Any]) -> dict[str, Any]:
    ...
    specs = build_boot_specs(authorization, REPO_ROOT / V12_OUTPUT_PATH)
...
def validate_authorization_v12(package: Mapping[str, Any]) -> dict[str, Any]:
    """Validate one V12 package without touching a GPU.

    Args:
        package: The parsed V12 authorization document.

    Returns:
        A non-execution summary of the checks that passed.

    Raises:
        B0RunAuthorizationV12Error: If any registered contract failed.
    """
    ...
    try:
        ...
        raise B0RunAuthorizationV12Error(str(exc)) from exc
    lane_coverage = _validate_lane_coverage(package)
    spec_lanes = _validate_spec_lanes(package)
    _require(
        _reviewed_output_path(REPO_ROOT / V12_AUTHORIZATION_PATH)
        == REPO_ROOT / V12_OUTPUT_PATH,
        "V12 does not resolve the reviewed create-new output pair",
    )
    try:
    ...
    return {
        "artifact_id": "p4-b0-run-authorization-v12-validation",
        "status": "pass",
        "gpu_executed": False,
...
def main() -> int:
    """Validate one V12 package and emit its non-execution summary."""
    args = parse_args()
    result = validate_authorization_v12(_load_json(args.authorization))
...
if __name__ == "__main__":
    ...
    except B0RunAuthorizationV12Error as exc:
        print(f"V12 authorization refused: {exc}", file=sys.stderr)
