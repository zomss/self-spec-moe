#!/usr/bin/env python3
"""E3: Qwen3-32B beta column (77 harness, new MODELS entry via driver).
...
SA.MODELS["q3_32b"] = dict(hf="Qwen/Qwen3-32B", trust=False, moe=False,
                          layers=64)
CFG = SA.MODELS["q3_32b"]
REFDIR = PHASE / "data/refs/q3_32b_c16384"
OUT = PHASE / "data/beta_q3_32b.csv"
...
def append_row(row, extra=None):
    ...
    with OUT.open("a") as f:
        ...
        if not exists:
            w.writeheader()
        w.writerow(row)
    print(f"[q3-32b] {row['arm']}: beta={row['beta_greedy']}", flush=True)
...
def main() -> int:
    ...
    if a.stage == "singles":
        ...
        # ranking prior only: 6 prompts (576 pos); greedy re-measures at 12
        args.prompts = 6
        ...
        for rnd in range(1, BUDGET + 1):
            ...
            print(f"[q3-32b] greedy round {rnd}: set {sorted(chosen)} "
