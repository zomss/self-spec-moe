"""W7-FP4 losslessness spot-check (W0/W2 standard).

Greedy DP=2+EP forced-PCIe. Generates the same 16 prompts under:
  nospec  : plain full-EP greedy (the reference distribution).
  bf16    : full-replica bf16 draft spec (the W7 baseline; draft == target ckpt).
  nvfp4   : full-replica NVFP4 draft spec (this stage; draft = NVFP4 ckpt).
...
argv[1] one of: nospec | bf16 | nvfp4  (run serially, one engine at a time).
Writes /data/.../data/w7fp4_lossless_<mode>.json with per-prompt token_ids.
"""
import json
import os
import sys
...
OUT_DIR = "/data/smcho/ssm-w7fp4/research/34_worldA_system/data"
OUTLEN = 64
...
def worker(rank, master_ip, master_port, mode):
    ...
    if spec:
        draft = NVFP4_CKPT if mode == "nvfp4" else MODEL
        sc = {"method": "draft_model", "model": draft,
    ...
    if rank == 0:
        ...
        path = os.path.join(OUT_DIR, f"w7fp4_lossless_{mode}.json")
...
def main():
    mode = sys.argv[1]
    assert mode in ("nospec", "bf16", "nvfp4")
