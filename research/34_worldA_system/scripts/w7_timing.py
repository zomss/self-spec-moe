"""W7: wall-clock decode tokens/s -- World A (bf16 full replica) vs no-spec.

Steady-state DECODE throughput on the comm-bound (forced-PCIe) DP=2+EP layout,
DeepSeek-V2-Lite, native deepseek_v2, greedy.

Two configs in one process tree (selected by argv):
  nospec  : plain full-EP decode, no speculative config.
  spec    : World A draft_model spec, VLLM_SELF_SPEC_DRAFT_FULL_REPLICA=1
            (bf16 full-replica comm-free draft), producer ON.

Decode-time isolation (two-length slope): for each batch we time a long run
(max_tokens=OUTLEN) and a short run (max_tokens=SHORTLEN). Both share the same
prefill, so decode_time = t(OUTLEN) - t(SHORTLEN) isolates (OUTLEN-SHORTLEN)
steady-state decode steps per sequence and cancels prefill exactly. tok/s =
(batch * (OUTLEN-SHORTLEN)) / decode_time  (system throughput, OUTPUT tokens).
We warm up both shapes per batch, then run ITERS timed iterations (mean/std).

A single LLM instance is built per (config, K) and reused across all batches and
prompt lengths to amortize model load. Spec sweeps K via W7_KS; nospec ignores K.

Env knobs:
  W7_MODE      nospec | spec            (also argv[1])
  W7_BATCHES   comma list, default 8,32,64,128,256
  W7_KS        comma list (spec only),  default 2,3,4,5
  W7_OUTLEN    long output length,       default 128
  W7_SHORTLEN  short output length,      default 32
  W7_ITERS     timed iterations,         default 3
  W7_WARMUP    warmup iterations,        default 1
  W7_OUT       output dir
  W7_GPU_MEM   gpu_memory_utilization,   default 0.90
  W7_EAGER     1 -> enforce_eager (no CUDA graphs); 0 -> graphs.  default 1
  W7_A2A_US    emulated exposed per-collective A2A latency (us).  default 0
  W7_NVLINK    1 -> do NOT force PCIe (caller controls NCCL env)  [labeling only]
"""
import json
import os
import sys
...
OUT_DIR = os.environ.get("W7_OUT", "/data/smcho/self-spec-moe/research/34_worldA_system/data")
OUTLEN = int(os.environ.get("W7_OUTLEN", "128"))
SHORTLEN = int(os.environ.get("W7_SHORTLEN", "32"))
ITERS = int(os.environ.get("W7_ITERS", "3"))
WARMUP = int(os.environ.get("W7_WARMUP", "1"))
GPU_MEM = float(os.environ.get("W7_GPU_MEM", "0.90"))
BATCHES = [int(x) for x in os.environ.get("W7_BATCHES", "8,32,64,128,256").split(",")]
KS = [int(x) for x in os.environ.get("W7_KS", "2,3,4,5").split(",")]
NVLINK = os.environ.get("W7_NVLINK", "0") == "1"
EAGER = os.environ.get("W7_EAGER", "1") == "1"
# Emulated exposed per-collective A2A latency (us), injected on the GPU stream
# (captured into CUDA graphs). 0 = native fabric only. The comm-free draft skips
# the AgRs collectives so this delay hits only the full-EP verify / nospec decode.
A2A_US = float(os.environ.get("W7_A2A_US", "0"))

# Fixed prompt; identical across configs/batches. ~16 tokens of prefix.
BASE_PROMPT = (
...
def worker(rank, dp, tp, master_ip, master_port, mode, k, q):
    ...
    kwargs = dict(
        ...
        enforce_eager=EAGER,
        # Stat logging must stay on so spec accept-length metrics are queryable.
    ...
    def run_batch(batch, out_len):
        # Distinct prompts per seq to avoid prefix-cache sharing across the batch.
    ...
    for batch in BATCHES:
        try:
            # Two-length slope method: decode_time per (L_long-L_short) steps
            # cancels the (shared) prefill cost and avoids the fragile
            # max_tokens=1 path. Warmup covers BOTH output-length shapes.
            ...
            # Accept-length only over the timed decode runs.
            snap0 = _snap()
            ...
            # decode_time for (OUTLEN - SHORTLEN) decode steps per sequence.
            ...
            # Suspect a CUDA-graph-capture leak into a timed run when the slope
            # decode is implausibly small vs the short-run time, or when the
            # per-iter tok/s spread is large (>15%). Such rows are not trusted.
            suspect = bool(
            ...
            if rank == 0:
                ...
                al_s = f" accept_len={row.get('accept_len'):.3f}" if spec else ""
                print(f"[W7 {tag}] batch={batch} decode={dt_m:.3f}s "
                      f"tok/s={tps_m:.1f}+-{tps_s:.1f}{al_s}", flush=True)
        except Exception as e:  # OOM or other
            if rank == 0:
                results.append({"batch": batch, "error": repr(e)[:300]})
                print(f"[W7] batch={batch} FAILED: {repr(e)[:200]}", flush=True)
            # An OOM likely corrupts the engine; stop this instance.
            break
...
def main():
    ...
    link = "nvlink" if NVLINK else "pcie"
    # Distinguish runs by execution mode + emulated A2A latency.
    suffix = ("eager" if EAGER else "cg")
    if A2A_US > 0:
        suffix += f"_a2a{int(A2A_US)}us"
    all_out = {"mode": mode, "link": link, "dp": DP, "tp": TP,
    ...
    for k in ks:
        ...
        path = os.path.join(OUT_DIR, f"w7_{mode}_{link}_{suffix}_{ktag}.json")
        with open(path, "w") as f:
            json.dump({**all_out, "this_k": k, "results": res}, f, indent=2)
        print(f"[W7] wrote {path}", flush=True)

    path = os.path.join(OUT_DIR, f"w7_{mode}_{link}_{suffix}.json")
    with open(path, "w") as f:
