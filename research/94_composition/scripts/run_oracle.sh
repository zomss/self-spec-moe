...
set -uo pipefail
WIN_FULLCG=1   # GQA arches use the FULLCG scratchpad; MLA/MoE do not
...
case "$ARCH" in
  dense)
    ...
    ;;
  mla)
    # MLA: scratchpad/FULLCG is GQA-only -> windows run PLAIN (matches C1
    # Stage A). TP1.
...
STRIDE="${ORACLE_STRIDE:-1}"; OFFSET="${ORACLE_OFFSET:-0}"; IDX=0
for q in none $Q1_NAME $Q2_NAME; do
  ...
  for w in none 512 2048; do
    if [ "$w" = none ]; then we=""; else
      we="VLLM_SELF_SPEC_DRAFT_KV_WINDOW=$w VLLM_SELF_SPEC_DRAFT_KV_SINKS=16"
      [ "$WIN_FULLCG" = "1" ] && we="$we VLLM_SELF_SPEC_DRAFT_FULLCG=1"
    fi
    for s in none b2; do
