...
| p75 e2 quant arms (hardware axis) | W4A16-asym / W4A16-sym / W8INT8 share one dir | W8 boots wrecked (num_accepted 0.0, accept_len null); asym boots exposed, no data committed | RESOLVED — BOUNDED, no published number affected. Compile-vs-load timeline: W4A16-sym was the primer and every sym boot compiled or loaded its OWN graph, and sym carries ALL published p75 numbers (tau 3.52/4.71/5.96 and the hardware-axis e2e 0.764/0.857/1.224x). The corrupted W8 zero-accept data was never cited (the only W8 tau in any doc is the paper's OWN Tab.4 value); asym produced no committed artifacts. Re-measure only if a measured W8-on-Qwen2.5 number is ever needed |
| p92 e2_sweep (RL drafter refresh) | 92-drafter-step{8,16} <-> static W4A16 (2 dirs) | none | RESOLVED — BENIGN. The refresh drafters loaded the static draft's graph, but both are W4A16-INT4-sym quantizations of the same architecture: the compiled graph is structurally identical and weights are runtime buffers. Acceptance screen agrees: refresh-drafter accepts 3.8-4.0, inside the physical band and matching the seed anchors (4.10 at step 0); the E2 staleness-curve finding stands |
